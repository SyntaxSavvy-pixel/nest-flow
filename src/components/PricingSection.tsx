...
const PricingSection = () => {
  const renderCell = (value: boolean | string) => {
    if (typeof value === "boolean") {
      return value ? <div className="flex items-center justify-center">
          <Check className="w-4 h-4 text-teal" />
        </div> : <span className="text-muted-foreground/30">—</span>;
    }
    return <span className="text-xs text-muted-foreground">{value}</span>;
  };
  return <section id="pricing" className="py-20 md:py-24 bg-cream-dark relative overflow-hidden">
      <div className="container mx-auto px-6 relative z-10">
        <ScrollReveal className="text-center mb-12">
          <motion.span className="inline-block text-sm font-medium text-teal uppercase tracking-wider mb-3" initial={{
          opacity: 0,
          y: 10
        }} whileInView={{
          opacity: 1,
            ...
            <table className="w-full">
              ...
              <thead>
                <tr className="border-b border-border">
                  ...
                  {plans.map(plan => <th key={plan.name} className={`p-4 text-center w-[20%] ${plan.featured ? 'bg-primary/5' : ''}`}>
                      {plan.limited}
                      {plan.featured && <span className="inline-block px-2 py-0.5 bg-primary text-primary-foreground text-[10px] font-medium rounded-full mb-1">
                          Popular
                        </span>}
              ...
              {/* Feature Rows */}
              <tbody>
                {features.map((feature, idx) => <tr key={feature.name} className={`border-b border-border/50 ${idx % 2 === 0 ? 'bg-secondary/5' : ''}`}>
              ...
              <tfoot>
                <tr>
                  ...
                  {plans.map(plan => <td key={plan.name} className={`p-4 text-center ${plan.featured ? 'bg-primary/5' : ''}`}>
                      <Button variant={plan.featured ? "default" : "outline"} size="sm" className={`w-full text-xs ${plan.featured ? 'gradient-primary text-primary-foreground' : ''}`}>
                        {plan.cta}
                      </Button>
                    </td>)}
                </tr>
